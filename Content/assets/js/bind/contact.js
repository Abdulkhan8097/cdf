$(document).ready(function () {
    ...
    var $validator = $('form#contact_form').validate({
        rules: {
            Name: {
                required: true
            },
        ...
        submitHandler: function (form) {
            debugger;
            var $valid = $('form#contact_form').valid();
            if (!$valid) {
...
function AddLeads() {
    ...
    if (window.FormData !== undefined) {
        ...
        var inp = $("form#contact_form input").length + $("form#contact_form select").length;
        var other_data = $('form#contact_form').serializeArray();
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result == true) {
                    $("form#contact_form").get(0).reset();
                    $.dialog({
                        title: false,
                        content: '<p  style="font-size:18px;text-align:center;">Thankyou for your contacting us our executive will contact you soon.</p>',
                        closeIconClass: 'invisible-close',
                    });
                    setTimeout(function () {
                        $(".invisible-close").click();
                    }, 5000);
                    $("button[type=submit]").html("Send your message");
                }
